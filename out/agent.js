...
function detectTrigger(output) {
    if (/NEXT:생성자/i.test(output))
        return "generator";
    if (/NEXT:평가자/i.test(output))
        return "evaluator";
    if (/NEXT:계획자/i.test(output))
        return "planner";
    if (/\bDONE\b/.test(output))
        return "done";
    if (/STOP:사용자보고/i.test(output))
        return "stop";
    if (/FEEDBACK:반영중/i.test(output))
        return "feedback";
    return null;
}
// ──────────────────────────────
...
async function runEvaluator(memory, retryCount) {
    ...
    const userMessage = `현재 재시도 횟수: ${retryCount}/${exports.MAX_RETRY}\n평가를 시작해줘. 정적 분석과 CMD 실행 테스트를 반드시 수행해.`;
    return (0, llm_1.callLLM)(systemPrompt, userMessage);
}
