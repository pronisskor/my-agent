...
function detectTrigger(output) {
    ...
    for (const line of lines) {
        if (line.includes("NEXT:생성자")) {
            return "generator";
        }
        if (line.includes("NEXT:평가자")) {
            return "evaluator";
        }
        if (line.includes("NEXT:계획자")) {
            return "planner";
        }
        if (line.includes("STOP:사용자보고")) {
            return "stop";
        }
...
${plan || "(없음)"}
`.trim();
    return await (0, llm_1.callLLM)(systemPrompt, userInput);
}
// ──────────────────────────────
// 생성자
// ──────────────────────────────
...
${errorReport || "(없음)"}
`.trim();
    return await (0, llm_1.callLLM)(systemPrompt, "기획서를 읽고 코드 작성을 시작해.");
}
// ──────────────────────────────
// 평가자
// ──────────────────────────────
...
작업 시작 전 읽은 파일명을 첫 줄에 반드시 나열해.
현재 재시도 횟수: ${retryCount}/${exports.MAX_RETRY}
오류 있으면: 오류보고서.txt 내용을 작성하고 마지막 줄에 NEXT:생성자 출력 (${exports.MAX_RETRY}회 초과 시 STOP:사용자보고)
...
${errorReport || "(없음)"}
`.trim();
    return await (0, llm_1.callLLM)(systemPrompt, "코드를 평가하고 오류를 보고해.");
}
// ──────────────────────────────
// 메모리 저장
// ──────────────────────────────
